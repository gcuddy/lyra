...
export default function TopBar() {
  ...
  useEffect(() => {
    ...
    setPlaying(true);
  }, [loadedSong, setPlaying, setTimeListenedAtom]);
  ...
  const scrobbleFn = useCallback((done: boolean) => {
    if (!loadedSong || !lastFm) return;
        ...
        <div className="flex grow h-14 border border-app-line bg-app-box/50 rounded-lg overflow-hidden self-center">
          ...
          {!!loadedSong && (
            ...
            <div className="pointer-events-auto leading-3 select-none cursor-default text-center flex flex-col text-sm items-center justify-center grow w-full p-1">
              ...
              <TimeDisplay audioRef={audioRef} scrobbleFn={scrobbleFn} />
            </div>
          )}
        </div>
      </div>
...
function TimeDisplay({
  ...
  useEffect(() => {
    ...
    if (timeListened >= duration / 2 || timeListened >= 4 * 60) {
      ...
      // scrobbleFn();
      console.log("should scrobble");

    } else {
      setShouldScrobble(false);
    }
  }, [duration, timeListened, setShouldScrobble])
  ...
  useEffect(() => {
    ...
    function timeupdate() {
      ...
      setTimeListened(prev => prev + Math.floor(currentTime) - time);
    ...
    const _setDuration = () => setDuration(audio.duration);
    audio.addEventListener("timeupdate", timeupdate);
    audio.addEventListener("loadedmetadata", _setDuration);

    return () => {
      audio.removeEventListener("timeupdate", timeupdate);
      audio.removeEventListener("loadedmetadata", _setDuration);
    };
  }, [audioRef, setNextSong, onEnded, setTimeListened, time]);
